...
if __name__ == '__main__':
    ...
    for idx in range(0,len(audio.samples),step):
        ...
        if previous is not None:
            ...
            match_idx = np.argmin([np.abs(m.a - round(len(left.text) / 2)) for m in matches])
            glue_match = matches[match_idx]
            print(glue_match)
            glue_idx_left= glue_match.a
            glue_idx_right= glue_match.b
            letters_left = [LetterIdx(x.letter, x.index + idx - step) for x in left.seq if
                     x.index < glue_idx_left]
            letters_right = [LetterIdx(x.letter, x.index + idx) for x in right.seq if
                      x.index >= glue_idx_right]
            letters.extend(letters_left)
            letters.extend(letters_right)
            print(f"left: {left.text}, right: {right.text}")
            print(f"GLUED left: {AlignedTranscript(letters_left).text}, right: {AlignedTranscript(letters_right).text}")
        else:
            right=AlignedTranscript(seq=[s for s in ts.seq if s.index<step])
